...
3. **Комментарии:** Щедрые и качественные комментарии на русском (объясняй "почему", а не только "что").
   - Не удалять уже существующие в коде комментарии, если они не противоречат логике.
   - После определения функции, класса или модуля должны быть **Docstrings**. Следуй PEP 257 для **Docstrings**.
   - Используй TODO-комментарии для обозначения незавершённых участков кода.
...
4. **История:** AI Ассистент всегда неукоснительно ориентируется на **последний коммит** в ветке `main` проекта https://github.com/pooow/cpro-autotest.
